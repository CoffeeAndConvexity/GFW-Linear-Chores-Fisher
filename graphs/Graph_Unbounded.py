...
def draw_ascent_direction(beta_1, beta_2, p, with_point=False, color=['blue', 'orange'], target=None):
    ...
    if target is None:
        ...
        a = Arrow3D([beta_1, target[0]], [beta_2, target[1]], [p, target[2]], mutation_scale=8, lw=3, arrowstyle="-|>", color=color[0])
    ax.add_artist(a)
...
draw_ascent_direction(1, 2, 2, with_point=True, color=['blue', 'orange'])
...
def gradient_ascent(x0, eta, A, b, num_iter=5):
    ...
    for i in range(num_iter):
        ...
        if np.linalg.norm(x) < 1e-7:
            ...
            draw_ascent_direction(x_[0], x_[1], x_[2], with_point=True, color=['green', 'green'], target=x)
            x_ = x

A, b = polytope_dual_A_b_form_with_red_beta_p(2, 1, np.array([[2], [1]]), np.array([1, 1, 1]))
# draw paths generated by gradient ascent algorithm
gradient_ascent(x0=np.array([1.1, 2.1, 2]), eta=3, A=A, b=b, num_iter=5)
gradient_ascent(x0=np.array([1, 2, 1.8]), eta=3, A=A, b=b, num_iter=10)
...
ax.set_xlabel(r"$\beta_1$", size=25, fontweight='bold')
ax.set_ylabel(r"$\beta_2$", size=25, fontweight='bold')
ax.set_zlabel(r"$p$", size=25, fontweight='bold')

plt.draw()
plt.tight_layout()
ax.view_init(elev=10., azim=-50.)
plt.savefig("./Graph_Unbounded.png", bbox_inches='tight')