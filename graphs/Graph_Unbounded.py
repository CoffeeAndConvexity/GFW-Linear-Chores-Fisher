...
def gradient_ascent(x0, eta, A, b, num_iter=5):
    ...
    for i in range(num_iter):
        ...
        if np.linalg.norm(x) < 1e-7:
            ...
            draw_ascent_direction(x_[0], x_[1], x_[2], with_point=True, color=['olive', 'olive'], target=x)
            x_ = x

A, b = polytope_dual_A_b_form_with_red_beta_p(2, 1, np.array([[2], [1]]), np.array([1, 1, 1]))
# draw paths generated by gradient ascent algorithm
gradient_ascent(x0=np.array([1.1, 2.1, 2]), eta=1, A=A, b=b, num_iter=15)
gradient_ascent(x0=np.array([1, 2, 1.8]), eta=1, A=A, b=b, num_iter=15)
...
plt.savefig("./Graph_Unbounded.png")