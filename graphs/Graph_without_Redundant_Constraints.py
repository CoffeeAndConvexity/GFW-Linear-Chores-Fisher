...
def projection(x, A, b):
    ...
    objective = cp.Minimize(cp.norm(y - x))
    constraints = [A @ y <= b]
...
def gradient_descent(x0, eta, A, b, num_iter=5):

    x_ = x = x0
    for i in range(num_iter):
        x = projection(x + eta * np.array([-1 / x[0], -1 / x[1], 1]), A, b)
        draw_descent_direction(x_[0], x_[1], x_[2], with_point=True, color=['orange', 'orange'], target=x)
...
A, b = polytope_dual_A_b_form_with_red_beta_p(2, 1, np.array([[2], [1]]), np.array([1, 1, 1]))
gradient_descent(x0=np.array([1.1, 2.1, 2]), eta=1, A=A, b=b, num_iter=15)
gradient_descent(x0=np.array([1, 2, 1.8]), eta=1, A=A, b=b, num_iter=15)
